...
var users_urls = document.location.protocol + '//otherusers.json';
var userlist_url = document.location.protocol + '//users.json';
...
function add_event_listeners() {
    var ring_link_rand = document.getElementById('multiuser'),
        rand_box_link  = document.getElementById('random_tildebox'),
        ring_link_next = document.getElementById('multiuser_next');

    ring_link_rand.addEventListener('click', link_multiuser);

    add_tilde_ring_listener_if_present( rand_box_link );
    add_tilde_ring_listener_if_present( ring_link_next );
}

function link_multiuser() {
	if (document.getElementById('random_tildebox')) {link_random_user();};
    link_users();
}
    ...
    fetchJSONFile( userlist_url,
                   function(users) {
                       ...
                       var user_url = users[user()].homepage,
                           ...
                           random_user_link = document.getElementById('multiuser'),
                           next_user_link   = document.getElementById('multiuser_next');
...
function normalize_tilde_urls(urls) {
    ...
    var trimmed_urls  = urls.map(trim_trailing_slash),
        host_users = window.location.hostname;

    return remove(trimmed_urls, host_users);
}
...
function add_tilde_ring_listener_if_present(link) {
    if (link) {
        link.addEventListener('click', multiuser);
    }
}
